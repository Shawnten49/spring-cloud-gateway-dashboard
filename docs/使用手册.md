...
- 用户管理：ADMIN 可新增、屏蔽/启用用户（屏蔽即吊销其 token；`admin` 特殊用户不可屏蔽；不允许删除）
- 查看路由：路由列表、详情、当前生效路由、网关健康状态
- 动态修改：新建、编辑、停用/启用、删除路由
- 保存即生效：保存后无需重启网关，新配置立即生效（内嵌网关另有 5 秒轮询兜底）
- 强校验：保存前校验字段、工厂名、参数绑定，错误配置上不了线
- 全程审计：谁、何时、改了什么、变更前后内容、来源 IP
- 安全加固：改密/屏蔽后旧 token 立即失效（吊销）、登录限流（防暴力破解）、统一 JSON 错误响应
...
左侧菜单 **用户管理**（仅 ADMIN 可见）：管理登录账号。

- **查看**：列表展示用户名、角色、状态（启用/已屏蔽）、创建/更新时间；顶部搜索框按用户名模糊搜索
- **新建用户**：点击右上角"新建用户"，填写用户名（不可重复）、密码（至少 8 位）、角色（ADMIN 管理员 / VIEWER 只读）；创建后立即可登录
- **屏蔽 / 启用**：行操作按钮切换状态。屏蔽后该用户**无法登录**且**已签发的 token 立即失效**；被屏蔽用户保留在列表中，可随时启用（启用后需重新登录）
- **admin 特殊保护**：`admin` 行的"屏蔽"按钮禁用（悬浮提示"特殊用户不可屏蔽"），后端也会强制拒绝
- **不允许删除**：系统不提供删除用户的功能（无删除接口）
...
当前账号是 `VIEWER`（只读）。使用 `admin` 账号，或让管理员在**用户管理**页为你调整/新建账号。修改角色暂未提供（需改库，见 §12）。
...
| `RouteSyncScheduler` | 每 5 秒比较**全局修订号水印**（`config_revision` 表，写操作单调递增），变化即刷新 |
...
- **轮询兜底**：即使推送失败（网关短暂不可达等），网关每 5 秒检查一次**全局修订号水印**（每次写操作单调递增），最多延迟一个轮询周期生效
- **推送与轮询不重复**：推送刷新成功后会把当前修订号同步给轮询器，同一次变更不会重复触发刷新；之后若修订号再次变化，轮询仍会正常兜底
...
当前为 v1（单实例进程、保存即生效、无草稿），已知边界：

- 单实例进程；多实例刷新传播已具备**数据库修订号轮询兜底**（内嵌网关与外部网关均 ≤5s 生效），低延迟广播（Redis pub/sub）仍为可选演进
- 无草稿/发布两阶段流程，误操作靠审计日志追溯
- 只管理路由本身，不管理全局过滤器、CORS、TLS 等网关级配置
- 用户管理仅支持**新增 / 屏蔽 / 启用**；修改角色、重置他人密码暂未提供（需改库）

扩展方向（均可基于现有架构演进）：低延迟刷新广播（Redis pub/sub）、草稿与回滚、配置中心（Nacos）作为真源的适配、用户角色管理与密码重置、路由匹配预览（输入路径模拟命中）。
